...
// following pragmas added by Gwilym to ignore warnings in this code
...
static int read_chunk_bytes(spng_ctx *ctx, uint32_t bytes)
{
    ...
    if (bytes > ctx->cur_chunk_bytes_left)
        return 1; /* XXX: more specific error? */
...
static int read_idat_bytes(spng_ctx *ctx, uint32_t *bytes_read)
{
    ...
    { /* TODO: calculate bytes to read for progressive reads */
        len = SPNG_READ_SIZE;
        if (len > ctx->current_chunk.length)
...
{
    if (ctx == NULL || stream == NULL || dest == NULL)
        return 1;
...
{
    if (ihdr->width > png_u32max || ihdr->width > max_width || !ihdr->width)
        return SPNG_EWIDTH;
    if (ihdr->height > png_u32max || ihdr->height > max_height || !ihdr->height)
        return SPNG_EHEIGHT;
...
static int check_offs(const struct spng_offs *offs)
{
    ...
    if (offs->x < png_s32min || offs->y < png_s32min)
        return 1;
...
static int read_chunks_before_idat(spng_ctx *ctx)
{
    ...
    while (!(ret = read_header(ctx, &discard)))
    {
        ...
        {
            if (ctx->file.plte && chunk.offset > ctx->plte_offset)
                return SPNG_ECHUNK_POS;
            if (ctx->file.chrm)
        ...
        {
            if (ctx->user.splt)
                continue; /* XXX: should check profile names for uniqueness */
            if (!chunk.length)
                return SPNG_ECHUNK_SIZE;
            ...
            if (check_png_keyword(ctx->splt_list[i].name))
                return SPNG_ESPLT_NAME;

            keyword_len = strlen(ctx->splt_list[i].name);

            if ((chunk.length - keyword_len - 1) == 0)
                return SPNG_ECHUNK_SIZE;
            ...
            if (ctx->n_splt > 1)
            {
                ...
                for (j = 0; j < i; j++)
                {
                    if (!strcmp(ctx->splt_list[j].name, ctx->splt_list[i].name))
            ...
            if (ctx->splt_list[i].n_entries == 0)
                return SPNG_ECHUNK_SIZE;
            if (sizeof(struct spng_splt_entry) > SIZE_MAX / ctx->splt_list[i].n_entries)
                return SPNG_EOVERFLOW;
    ...
    while (!(ret = read_header(ctx, &discard)))
    {
        ...
        if (is_critical_chunk(&chunk))
        {
            ...
            { /* ignore extra IDATs */
                ...
                return SPNG_ECHUNK_POS; /* critical chunk after last IDAT that isn't IEND */
        }
        ...
        if (!memcmp(chunk.type, type_chrm, 4))
            return SPNG_ECHUNK_POS;
        else if (!memcmp(chunk.type, type_gama, 4))
            return SPNG_ECHUNK_POS;
        else if (!memcmp(chunk.type, type_iccp, 4))
...
static int calculate_subimages(struct spng_subimage sub[7], size_t *widest_scanline, struct spng_ihdr *ihdr, unsigned channels)
{
    ...
    { /* Calculate scanline width in bits, round up to the nearest byte */
        ...
        if (scanline_width > SIZE_MAX / ihdr->width)
            return SPNG_EOVERFLOW;
        ...
        if (widest < scanline_width)
            widest = scanline_width;
    }
...
int spng_decode_image(spng_ctx *ctx, unsigned char *out, size_t out_size, int fmt, int flags)
{
    ...
    if (ctx->ihdr.bit_depth < 8)
        ...
        bytes_per_pixel = channels * (ctx->ihdr.bit_depth / 8);
    ...
    int apply_trns = 0;
    if (flags & SPNG_DECODE_USE_TRNS && ctx->stored.trns)
        apply_trns = 1;

    int apply_gamma = 0;
    if (flags & SPNG_DECODE_USE_GAMA && ctx->stored.gama)
        apply_gamma = 1;
    ...
    if (interlaced)
        row = spng__malloc(ctx, out_width);
    else
        row = out;
    ...
    for (pass = 0; pass < 7; pass++)
    {
        ...
        for (scanline_idx = 0; scanline_idx < sub[pass].height; scanline_idx++)
        {
            ...
            for (k = 0; k < width; k++)
            {
                ...
                else if (ctx->ihdr.color_type == SPNG_COLOR_TYPE_GRAYSCALE)
                {
                    if (ctx->ihdr.bit_depth == 16)
                    {
                        ...
                        if (apply_trns && ctx->trns.gray == gray_16)
                            a_16 = 0;
                        else
                            a_16 = 65535;
                    ...
                    else /* <= 8 */
                    {
                        ...
                        if (apply_trns && ctx->trns.gray == gray_8)
                            a_8 = 0;
                        else
                            a_8 = 255;
            ...
            if (apply_trns)
                trns_row(row, scanline, trns_px, width, fmt, ctx->ihdr.color_type, ctx->ihdr.bit_depth);

            if (do_scaling)
                scale_row(row, width, fmt, processing_depth, &sb);

            if (apply_gamma)
                gamma_correct_row(row, width, fmt, gamma_lut);
            ...
            if (interlaced)
            {
                ...
                for (k = 0; k < width; k++)
                {
                    size_t ioffset = ((adam7_y_start[pass] + scanline_idx * adam7_y_delta[pass]) *
                                          ctx->ihdr.width +
                                      adam7_x_start[pass] + k * adam7_x_delta[pass]) *
                                     pixel_size;
...
void spng_ctx_free(spng_ctx *ctx)
{
    ...
    if (ctx->iccp.profile != NULL && !ctx->user.iccp)
        spng__free(ctx, ctx->iccp.profile);

    if (ctx->gamma_lut != NULL)
        spng__free(ctx, ctx->gamma_lut);
    ...
    if (ctx->text_list != NULL && !ctx->user.text)
    {
        ...
        {
            if (ctx->text_list[i].text != NULL)
                spng__free(ctx, ctx->text_list[i].text);
            if (ctx->text_list[i].language_tag != NULL)
                spng__free(ctx, ctx->text_list[i].language_tag);
            if (ctx->text_list[i].translated_keyword != NULL)
                spng__free(ctx, ctx->text_list[i].translated_keyword);
        }
...
int spng_set_image_limits(spng_ctx *ctx, uint32_t width, uint32_t height)
{
    ...
    if (width > png_u32max || height > png_u32max)
        return 1;
...
{
    if (ctx == NULL || chunk_size > png_u32max)
        return 1;
...
int spng_decoded_image_size(spng_ctx *ctx, int fmt, size_t *out)
{
    ...
    if (fmt == SPNG_FMT_RGBA8)
    {
        if (4 > SIZE_MAX / ctx->ihdr.width)
            return SPNG_EOVERFLOW;
        res = 4 * ctx->ihdr.width;

        if (res > SIZE_MAX / ctx->ihdr.height)
            return SPNG_EOVERFLOW;
        res = res * ctx->ihdr.height;
    }
    else if (fmt == SPNG_FMT_RGBA16)
    {
        if (8 > SIZE_MAX / ctx->ihdr.width)
            return SPNG_EOVERFLOW;
...
int spng_set_text(spng_ctx *ctx, struct spng_text *text, uint32_t n_text)
{
    ...
    for (i = 0; i < n_text; i++)
    {
        ...
        {
            if (check_png_text(text[i].text, text[i].length))
                return 1;
        }
